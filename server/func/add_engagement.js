...
let affectedClient = 0
let senderNameGlobal = "";
process.on("message", message => {
  db.conn.getConnection(async (err, connection) => {
    ...
    await addEngagement(message, connection).then((answer) => {
      ...
      connection.release();
      console.log("result with profile ID: ");
      console.log("affectedClient: "+affectedClient);
      console.log("senderName: "+senderNameGlobal);
      process.send({"affectedClient": affectedClient, "senderName": senderNameGlobal});
      process.exit();
    }).catch ((result) => {
      connection.release();
...
const addEngagement = async (dict, connection) => {
  ...
  var getSenderName = "select name from profile where profile_id = "+String(dict.engagement_profile_id);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    await connection.query(existsQuery, async (err, result) => {
      ...
      await connection.query(getAffectedClient, async (err, affectedResult) => {
        ...
        affectedResult = JSON.stringify(affectedResult);
        affectedResult = JSON.parse(affectedResult);
        affectedClient = affectedResult[0]['profile_id'];

        await connection.query(getSenderName, async (err, senderName) => {
          if(err){
            reject(err);
          }
          ...
          senderNameGlobal = senderName[0]['name'];
          ...
          if(!Object.keys(result).length){
            ...
            // Get the profile ID of the affected client
